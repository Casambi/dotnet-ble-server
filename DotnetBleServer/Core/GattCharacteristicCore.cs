...
{
  public delegate Task GattCharacteristicEventHandlerAsync(GattCharacteristicCore sender, GattCharacteristicValueEventArgs eventArgs);

  /// <summary>
  /// Adds events to IGattCharacteristic1.
  /// </summary>
  public class GattCharacteristicCore : IGattCharacteristic1, IDisposable
  {
    ~GattCharacteristicCore()
    {
      Dispose();
    }

    internal static async Task<GattCharacteristicCore> CreateAsync(IGattCharacteristic1 proxy)
    {
      var characteristic = new GattCharacteristicCore
      {
        m_proxy = proxy,
      };
